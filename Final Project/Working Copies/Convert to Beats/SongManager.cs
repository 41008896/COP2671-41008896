...
namespace RhythmGameStarter
{
    ...
    public class SongManager : MonoBehaviour
    {
        ...
        //[HideInInspector] public float secPerBeat;    // Duration of one beat in seconds removed for tick conversion
        [HideInInspector] public float songPosition;   // Current playback position in ticks (beats)
        ...
        [NonSerialized] public float MPTK_DeltaTicksPerQuarterNote;
        ...
        public void PlaySong(SongItem songItem)
        {
            ...
            //secPerBeat = 60.0f / currentSongItem.bpm; removed for tick conversion
            MPTK_DeltaTicksPerQuarterNote = midiFilePlayer.MPTK_DeltaTicksPerQuarterNote;
        ...
        void Update()
        {
            if (!songPaused && songHasStarted)
            {
                ...
                //songPosition = (float)midiFilePlayer.MPTK_Position / 1000f; // In seconds
                songPosition = midiFilePlayer.MPTK_TickCurrent; // in beats

                // Update track positions and trigger progress events
                float songPositionInBeats = (songPosition + delay) / (float)midiFilePlayer.MPTK_DeltaTicksPerQuarterNote;
                trackManager.UpdateTrack(songPosition, songPositionInBeats);
                onSongProgress.Invoke(songPosition);

                // Calculate and send normalized progress for UI
                //float normalizedProgress = songPosition / (midiFilePlayer.MPTK_DurationMS / 1000f); // seconds
                float normalizedProgress = songPosition / (float)midiFilePlayer.MPTK_TickLast; // beats
