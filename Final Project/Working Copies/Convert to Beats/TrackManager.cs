...
namespace RhythmGameStarter
{
    ...
    public class TrackManager : MonoBehaviour
    {
        ...
        //public void UpdateTrack(float songPosition, float songPositionInBeats)
        //{
        //    if (useNotePool)
        //        UpdateNoteInPool();
        ...
        //                var songPositionInBeats = (songPosition + songManager.delay) / secPerBeat; //seconds
            ...
            foreach (var track in tracks)
            {
                switch (syncMode)
                {
                    case SyncMode.Track:
                        ...
                        {
                            var syncPosY = -songPositionInBeats * beatSize + track.lineArea.transform.localPosition.y + hitOffset;
                            // Change from seconds to ticks/beats
                            target.Translate(new Vector3(0, -1, 0) * songManager.MPTK_DeltaTicksPerQuarterNote * beatSize * Time.deltaTime);
                            target.localPosition = new Vector3(0, (syncPosY + target.localPosition.y) / 2, 0);
                        }
                        else
                        {
                    ...
                    case SyncMode.IndividualNote:
                        foreach (Note note in track.runtimeNote)
                        {
                            ...
                            {
                                // Change from seconds to ticks/beats
                                var originalY = ((note.noteTime + songManager.MPTK_DeltaTicksPerQuarterNote)) * beatSize;
                                note.transform.localPosition = Vector3.LerpUnclamped(
                                    new Vector3(0, originalY, 0),
                                    new Vector3(0, hitOffset, 0),
                                    (songPosition + songManager.MPTK_DeltaTicksPerQuarterNote) / (note.noteTime + songManager.MPTK_DeltaTicksPerQuarterNote));
                            }
                            else
                            {
                                // Use beats directly instead of converting through seconds
                                var notePosInBeats = (songPosition - note.noteTime) / (float)songManager.MPTK_DeltaTicksPerQuarterNote;
                                var syncPosY = -notePosInBeats * beatSize;
                                note.transform.localPosition = new Vector3(0, syncPosY + hitOffset, 0);
                            }
                        }
                        break;
        ...
        private void InitNote(GameObject newNoteObject, SongItem.MidiNote note)
        {
            ...
            var time = note.time;
            //var beatUnit = time / songManager.secPerBeat; // seconds
            var beatUnit = time / (float)songManager.MPTK_DeltaTicksPerQuarterNote; // beats
            //pos.y = beatUnit * beatSize + (songManager.delay / songManager.secPerBeat * beatSize); //seconds
            pos.y = beatUnit * beatSize + (songManager.delay / (float)songManager.MPTK_DeltaTicksPerQuarterNote * beatSize); // beats
