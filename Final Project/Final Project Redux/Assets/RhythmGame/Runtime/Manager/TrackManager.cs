...
namespace RhythmGameStarter
{
    ...
    public class TrackManager : MonoBehaviour
    {
        ...
        private List<SongItem.MidiNote> droppedNotes = new List<SongItem.MidiNote>();
        private bool verbose = false;

        public enum SyncMode
        {
            Track, IndividualNote, MIDI
        }
        ...
        {
            //Debug.Log("Delay is " + songManager.delay);
            if (useNotePool)
                UpdateNoteInPool();
            ...
            foreach (var track in tracks)
            {
                switch (syncMode)
                {
                    ...
                    case SyncMode.IndividualNote:
                        ...
                        break;
                    case SyncMode.MIDI:
                        Debug.Log("Entering SyncMode MIDI");
                        var targetMIDI = track.notesParent;
                        var songPositionInBeatsMIDI = (songPosition + songManager.delay) / secPerBeat;
                        if (syncSmoothing)
                        {
                            var syncPosY = -songPositionInBeatsMIDI * beatSize + track.lineArea.transform.localPosition.y + hitOffset;
                            targetMIDI.Translate(new Vector3(0, -1, 0) * (1 / secPerBeat) * beatSize * Time.deltaTime);
                            //Smooth out the value with Time.deltaTime
                            // print(songPosition + " vs  " + syncPosY + " vs " + target.localPosition.y + " vs smooth " + (syncPosY + target.localPosition.y) / 2);
                            targetMIDI.localPosition = new Vector3(0, (syncPosY + targetMIDI.localPosition.y) / 2, 0);
                        }
                        else
                        {
                            targetMIDI.localPosition = new Vector3(0, -songPositionInBeatsMIDI * beatSize + track.lineArea.transform.localPosition.y + hitOffset, 0);
                        }
                        break;
                }
            }
        ...
        private void InitNotePool()
        {
            ...
            for (int i = 0; i < notePrefabMapping.notesPrefab.Count; i++)
            {
                ...
                {
                    var note = GetNewNoteObject(i);
                    if (note != null && verbose)
                    {
                        Debug.Log($"Note of type {i} initialized in the pool. Total Notes: {pooledNotes.Count}");
                    }
                }
            }
        }
        ...
        {
            Debug.Log("SetUpNotePool: Method called.");

            if (songManager == null)
            {
                Debug.LogError("SetUpNotePool: songManager is null. Ensure TrackManager.Init() is called.");
                return;
            }

            if (songManager.currnetNotes == null)
            {
                Debug.LogError("SetUpNotePool: songManager.currnetNotes is null. Check if notes are loaded correctly.");
                return;
            }
        ...
        private void UpdateNoteInPool()
        {
            foreach (var track in tracks)
            {
                ...
                foreach (var note in track.allNotes)
                {
                    ...
                    if (!note.created && songManager.songPosition + poolLookAheadTime >= note.time)
                    {
                        ...
                        if (verbose)
                        // Log only relevant info to prevent console flooding
                            Debug.Log($"Note created at time: {note.time}, y-position: {newNoteObject.transform.localPosition.y}");
                    }
                    else if (!note.created && songManager.songPosition + poolLookAheadTime < note.time)
                    {
                        // Add to dropped notes list for summary logging
                        if (!droppedNotes.Contains(note))
                        {
            ...
            if (verbose)
            {
                // Log a summary after processing all notes
                if (droppedNotes.Count > 0)
                {
                    Debug.Log($"Notes dropped during this frame: {droppedNotes.Count}");
                    droppedNotes.Clear(); // Clear the list for the next frame
                }
            }
        }
